...
class Library   
    ...
    def make_order(name, book_titles, book_authors, issue_dates)
        ...
        unless books_was_not_found == 0
          @not_satisfied_orders += books_was_not_found;                      
        else
    ...
    def best_reader
      @readers.max_by {|name, reader| reader.orders.size}.last
    end
    
    #what is the most popular book
    def most_popular
      @books.sort_by {|book| book.rate}.reverse
    end
    ...
    def count_people_ordered_most_popular_books(count_of_popular_books)
      ...
      people = Array.new
      most_popular_books[0..count_of_popular_books-1].each do |book|
